...
This article aims to address the above research gaps by developing a spatially distributed MPOPF framework. The bulk distribution grid is divided into multiple networked areas, each solving its own local MPOPF problem and periodically communicating the values of boundary variables with neighboring areas. The interaction between the areas are modelled by following the principles of ENApp distributed 
