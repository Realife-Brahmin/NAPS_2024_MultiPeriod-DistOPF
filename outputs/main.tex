...
The growing presence of battery-associated distributed energy resources in distribution networks necessitates the development of multi-period optimal power flow (MPOPF). Generally, the MPOPF frameworks are developed as mixed integer non-convex programming (MINCP) and solved centrally. However, the main limitation of centralized MPOPF (MPCOPF) is its longer solution time, a typical solution time is in the order of \(10^3\) to \(10^4\) seconds. To overcome such deficiencies, this article proposes a spatially distributed MPOPF (MPDOPF) 
